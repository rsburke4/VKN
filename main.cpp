...
private:
	...
	void createCommandBuffer() {
		...
		allocInfo.commandBufferCount = 1; //There could be more! At the end of the tutorial, it seems we've cut some corners
		//We were more careful about allocating space for things pased on the actual number of
		//assets previouwly. This may be worth fixing in the future
	...
	void drawFrame() {
		...
		uint32_t imageIndex;
		vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex); //Get next swapchain image

		//Record command buffer
		vkResetCommandBuffer(commandBuffer, 0); //Reset command buffer
